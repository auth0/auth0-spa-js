      ...
      ({
        ...
        describe(`when ${name}`, () => {
          it('returns the ID token', async () => {
            ...
            expect(await auth0.getIdToken()).toEqual('my_id_token');
          });
          ...
          it('returns the ID token claims with custom scope', async () => {
            ...
            expect(
              await auth0.getIdToken({ scope: 'scope1 scope2 scope3' })
            ).toEqual('my_id_token');
          });

          describe('when using refresh tokens', () => {
            it('returns the ID token with offline_access', async () => {
              const auth0 = setup({ scope: 'foo', useRefreshTokens: true });
              await login(auth0);

              expect(
                await auth0.getIdToken({ scope: 'foo offline_access' })
              ).toEqual('my_id_token');
            });

            it('returns the ID token with custom scope and offline_access', async () => {
              const auth0 = setup({
                scope: 'scope1',
                advancedOptions: {
                  defaultScope: 'scope2'
