...
> A PR that changes public API, configuration, or supported patterns is **not complete** until the relevant docs are updated in the same PR. The inline Boundaries → Always Do rule ("update README.md and EXAMPLES.md in the same PR") is the enforceable version of this; the tables below say *which* doc to touch.
