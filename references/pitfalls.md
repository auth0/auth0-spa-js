...
## Transport: `http.ts` and the worker / non-worker split

Low-level HTTP lives in `src/http.ts`, not `fetcher.ts`. It owns `fetchWithTimeout` (fetch with an abort-based timeout and retry) and DPoP header injection. The key export is `switchFetch()`, which routes a request either through the web worker or directly on the main thread depending on whether the worker is in use. Anyone touching transport, timeout, retry, or DPoP-on-the-wire behavior needs to start here.

The **worker / non-worker split** is the non-obvious part: the same logical operation (a token request) has two runtime code paths — one that proxies through `worker/token.worker.ts` (refresh-token + in-memory-cache mode, keeping tokens off the main thread) and one that runs inline. `switchFetch()` is the fork. When debugging a transport issue, first determine which path is active — a bug reproducing in one path may not reproduce in the other, and a "fix" applied to only one path leaves the other broken.
