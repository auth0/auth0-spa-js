...
class Referencer extends experimental_utils_1.TSESLintScope.Referencer {
  ...
  visitFunction(node) {
    ...
    // Process the name.
    if (
      type === experimental_utils_1.AST_NODE_TYPES.FunctionDeclaration &&
      id
    ) {
      upperScope.__define(
        id,
        new experimental_utils_1.TSESLintScope.Definition(
          'FunctionName',
          id,
          node,
          null,
      ...
      type === experimental_utils_1.AST_NODE_TYPES.FunctionExpression &&
      id
    ) {
    ...
    for (let i = 0; i < params.length; ++i) {
      this.visitPattern(
        ...
        (pattern, info) => {
          if (
            pattern.type !== experimental_utils_1.AST_NODE_TYPES.Identifier ||
            pattern.name !== 'this'
          ) {
            innerScope.__define(
              pattern,
              new experimental_utils_1.TSESLintScope.ParameterDefinition(
                pattern,
                node,
                i,
                info.rest
  ...
  TSEnumDeclaration(node) {
    ...
    if (id) {
      scope.__define(
        id,
        new experimental_utils_1.TSESLintScope.Definition('EnumName', id, node)
      );
    }
  ...
  TSEnumMember(node) {
    ...
    if (initializer) {
      scope.__referencing(
        id,
        experimental_utils_1.TSESLintScope.Reference.WRITE,
        initializer,
        null,
        false,
        true
