...
function listVersions() {
  ...
  return changeFiles
    .map(function(file) {
      return file.match(versionJsonFileReg);
    })
...
function listNextReleaseFiles() {
  nextReleaseFiles = fsSyncFromRoot('readdir', nextReleaseDir).map(function(
    file
  ) {
    return nextReleaseDir + file;
  });
...
function addVersionJSONToChangelog(version, changes) {
  ...
  logParts[0] =
    logParts[0].replace(versionMarkerReg, versionMarker.join(version)) +
    insertMarker;
