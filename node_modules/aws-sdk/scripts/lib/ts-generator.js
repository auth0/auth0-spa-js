...
TSGenerator.prototype.fillApiModelFileNames = function fillApiModelFileNames(
  ...
  modelPaths.forEach(function(modelFileName) {
    var match = modelFileName.match(
      /^(.+)(-[\d]{4}-[\d]{2}-[\d]{2})\.normal\.json$/i
    );
    if (match) {
      var model = match[1];
      // add version
...
TSGenerator.prototype.updateDynamoDBDocumentClient = function updateDynamoDBDocumentClient() {
  ...
  var docClientCustomCode = fs
    .readFileSync(
      path.join(this._sdkRootDir, 'lib', 'dynamodb', 'document_client.d.ts')
    )
    .toString();
  ...
  if (namespaceIndexStart >= 0 && namespaceIndexEnd >= 0) {
    ...
    lines.splice(
      namespaceIndexStart + 1,
      namespaceIndexEnd - namespaceIndexStart - 1,
      this.generateDocumentClientInterfaces(1)
    );
    var code = lines.join('\n');
    this.writeTypingsFile(
      'document_client',
      path.join(this._sdkRootDir, 'lib', 'dynamodb'),
      code
    );
  }
};
...
TSGenerator.prototype.generateDocumentClientInterfaces = function generateDocumentClientInterfaces(
  ...
  shapeKeys.forEach(function(shapeKey) {
    ...
    code += self.generateTypingsFromShape(
      dynamodbModel,
      shapeKey,
      modelShape,
      tabCount,
 ...
 */
TSGenerator.prototype.loadServiceApi = function loadServiceApi(
  serviceIdentifier
) {
  ...
  var serviceFilePath = path.join(
    this._apiRootDir,
    metadata[serviceIdentifier].api_path
  );
  ...
  if (metadata[serviceIdentifier].waiters_path) {
    waiterFilePath = path.join(
      this._apiRootDir,
      metadata[serviceIdentifier].waiters_path
    );
 ...
 */
TSGenerator.prototype.transformDocumentation = function transformDocumentation(
  documentation
) {
  if (!documentation) {
    return '';
 ...
 */
TSGenerator.prototype.generateDocString = function generateDocString(
  documentation,
  tabCount
) {
  tabCount = tabCount || 0;
  ...
  code +=
    this.tabs(tabCount) +
    ' * ' +
    this.transformDocumentation(documentation) +
    '\n';
  code += this.tabs(tabCount) + ' */\n';
  return code;
};
 ...
 */
TSGenerator.prototype.generateCustomConfigFromMetadata = function generateCustomConfigFromMetadata(
  serviceIdentifier
) {
...
TSGenerator.prototype.generateSafeShapeName = function generateSafeShapeName(
  name,
  blacklist
) {
  blacklist = blacklist || [];
...
TSGenerator.prototype.extractTypesDependOnStream = function extractTypesDependOnStream(
  shapeKey,
  modelShape
) {
  var streamTypeList = [];
  ...
  for (var subModelKey in modelShape) {
    ...
    var subStreamTypeList = this.extractTypesDependOnStream(
      subModelKey,
      subModel
    );
 ...
 */
TSGenerator.prototype.generateTypingsFromShape = function generateTypingsFromShape(
  model,
  shapeKey,
  shape,
  tabCount,
  ...
  if (type === 'structure') {
    ...
    memberKeys.forEach(function(memberKey) {
      ...
      memberType = self.generateSafeShapeName(
        memberType,
        [].concat(customClassNames, ['Date', 'Blob'])
      );
      code +=
        tabs(tabCount + 1) + memberKey + required + ': ' + memberType + ';\n';
    });
    code += tabs(tabCount) + '}\n';
  } else if (type === 'list') {
    code +=
      ...
      shapeKey +
      ' = ' +
      this.generateSafeShapeName(shape.member.shape, customClassNames) +
      '[];\n';
  } else if (type === 'map') {
    code +=
      tabs(tabCount) +
      ...
      shapeKey +
      ' = {[key: string]: ' +
      this.generateSafeShapeName(shape.value.shape, customClassNames) +
      '};\n';
    ...
    code +=
      tabs(tabCount) + 'export type ' + shapeKey + ' = ' + stringType + ';\n';
  } else if (
    [
      'double',
      'long',
 ...
 */
TSGenerator.prototype.generateTypingsFromOperations = function generateTypingsFromOperations(
  className,
  operation,
  operationName,
  tabCount
  ...
  operationName =
    operationName.charAt(0).toLowerCase() + operationName.substring(1);
...
TSGenerator.prototype.generateConfigurationServicePlaceholders = function generateConfigurationServicePlaceholders() {
  ...
  this.writeTypingsFile(
    'config_service_placeholders',
    path.join(this._sdkRootDir, 'lib'),
    code
  );
};

TSGenerator.prototype.getServiceApiVersions = function generateServiceApiVersions(
  serviceIdentifier
) {
  var metadata = this.metadata;
 ...
 */
TSGenerator.prototype.generateTypingsFromWaiters = function generateTypingsFromWaiters(
  className,
  waiterState,
  waiter,
  underlyingOperation,
  ...
  tabCount = tabCount || 0;
  var operationName =
    waiter.operation.charAt(0).toLowerCase() + waiter.operation.substring(1);
  ...
  code +=
    ...
    '", params: ' +
    inputShape +
    ' & {$waiter?: WaiterConfiguration}, callback?: (err: AWSError, data: ' +
    outputShape +
    ') => void): Request<' +
    outputShape +
    ', AWSError>;\n';
 ...
 */
TSGenerator.prototype.includeCustomService = function includeCustomService(
  serviceIdentifier
) {
 ...
 */
TSGenerator.prototype.generateCustomNamespaceTypes = function generateCustomNamespaceTypes(
  serviceIdentifier,
  className
) {
  var self = this;
  ...
  serviceInfo.forEach(function(data) {
    ...
    if (aliases.length) {
      importCode +=
        'import {' + aliases.join(', ') + "} from '../" + data.path + "';\n";
    }
  });
  code += '}\n';
  return {
...
TSGenerator.prototype.containsEventStreams = function containsEventStreams(
  model
) {
 ...
 */
TSGenerator.prototype.processServiceModel = function processServiceModel(
  serviceIdentifier
) {
  ...
  var customNamespaces = this.generateCustomNamespaceTypes(
    serviceIdentifier,
    className
  );
  var customClassNames = customNamespaces
    ? customNamespaces.customClassNames
    : [];
  ...
  var parentClass = hasCustomizations
  ...
  operationKeys.forEach(function(operationKey) {
    code += self.generateTypingsFromOperations(
      className,
      modelOperations[operationKey],
      operationKey,
      1
    );
  });
  ...
  waiterKeys.forEach(function(waitersKey) {
    ...
    code += self.generateTypingsFromWaiters(
      className,
      waitersKey,
      waiter,
      operation,
  ...
  code +=
    ...
    this.getServiceApiVersions(serviceIdentifier).join('"|"') +
    '"|"latest"|string;\n';
  ...
  code +=
    ...
    customConfigTypes.join(' & ') +
    ' & ClientApiVersions;\n';
  // export interfaces under Types namespace for backwards-compatibility
  code += this.generateDocString(
    'Contains interfaces for use with the ' + className + ' client.',
    1
  );
 ...
 */
TSGenerator.prototype.writeTypingsFile = function writeTypingsFile(
  name,
  directory,
  code
) {
