...
function ClientCreator() {
  ...
  this._serviceCustomizationsFolderPath = path.join(
    __dirname,
    '..',
    'lib',
    'services'
...
ClientCreator.prototype.getAllApiFilenamesForService = function getAllApiFilenamesForService(
  modelName
) {
  var serviceRegex = new RegExp(
    '(^' + modelName + '-([\\d]{4}-[\\d]{2}-[\\d]{2})\\.([\\w]+))\\.json$'
  );
  var modelRegex = /(([\d]{4}-[\d]{2}-[\d]{2})\.([\w]+))\.json$/;

  var models = {};
  var versions = {};
  this.getAllApiFilenames()
...
ClientCreator.prototype.customizationsExist = function customizationsExist(
  serviceName
) {
...
ClientCreator.prototype.generateClientFileSource = function generateClientFileSource(
  serviceMetadata,
  specifiedVersion
) {
  ...
  code +=
    ...
    "', ['" +
    []
      .concat(obsoleteVersions, versionNumbers)
      .sort()
      .join("', '") +
    "']);\n";
  ...
  versionNumbers.forEach(function(version) {
    ...
    code +=
      "Object.defineProperty(apiLoader.services['" +
      serviceName +
      "'], '" +
      version +
      "', {\n";
...
ClientCreator.prototype.generateDefinePropertySource = function generateDefinePropertySource(
  objName,
  serviceName,
  className
) {
...
ClientCreator.prototype.generateAllServicesSource = function generateAllServicesSource(
  services,
  fileName
) {
...
ClientCreator.prototype.writeClientServices = function writeClientServices() {
  ...
  var browserClientInfo = this.generateAllServicesSource(
    corsServices,
    'browser_default'
  );
