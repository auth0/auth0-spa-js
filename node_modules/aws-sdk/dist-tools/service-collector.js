...
function getServiceHeader(service) {
  if (service === 'all') {
    return Object.keys(serviceClasses)
      .map(function(name) {
  ...
  var versions = serviceClasses[service].apiVersions
  ...
  var file = util.format(
    "if (!Object.prototype.hasOwnProperty.call(AWS, '" +
...
function getService(service, version) {
  if (service === 'all') {
    return Object.keys(serviceClasses)
      .map(function(name) {
        var out = serviceClasses[name].apiVersions
          .map(function(svcVersion) {
  ...
  var line = util.format(
    ...
    service,
    svc.api.apiVersion,
    "require('../apis/" + serviceFileName + '-' + svc.api.apiVersion + ".min')"
  );
  lines.push(line);
  if (
    Object.prototype.hasOwnProperty.call(api, 'paginators') &&
    Object.keys(api.paginators).length
  ) {
    line = util.format(
  ...
  if (
    Object.prototype.hasOwnProperty.call(api, 'waiters') &&
    Object.keys(api.waiters).length
  ) {
    line = util.format(
