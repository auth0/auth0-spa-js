...
Pass `experiment_id` and `variation_id` via `authorizationParams` on a per-call basis. The override applies to that request only — the next login without these params reverts to normal server-side assignment.
...
Pass these **per-call** rather than in `authorizationParams` at client construction time, so the override does not affect silent `prompt=none` token-renewal calls (Experiment Center does not run on those).
